...
export default function QuizResult({
  ...
  const getBadge = (score) => {
    if (score >= 80) return <Badge className="bg-green-500/20 text-green-600">Excellent</Badge>;
    if (score >= 60) return <Badge className="bg-yellow-500/20 text-yellow-600">Good</Badge>;
    ...
    <div className="mx-auto space-y-6">
      ...
      <CardContent className="space-y-8">
        ...
        >
          ...
          <div className="pt-2">{getBadge(result.quizScore)}</div>
        </motion.div>
        ...
        >
          ...
          {result.questions.map((q, index) => (
            <motion.div
              key={index}
              className={`border rounded-lg p-4 space-y-3 transition-all duration-300 ${q.isCorrect ? "hover:bg-green-50/50" : "hover:bg-red-50/50"
                }`}
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}

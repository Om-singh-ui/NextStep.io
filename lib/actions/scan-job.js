...
async function buildScanResult({ scanId, finalScore, aiAnalysis, heuristicAnalysis, extractedText }) {
  ...
  if (aiAnalysis.ragContext) {
    result.ragInsights = {
      detectedCompanies: aiAnalysis.ragContext.companies,
      scamPatterns: aiAnalysis.ragContext.scams,
...
async function getEnhancedRiskLevel(score, aiAnalysis, heuristicAnalysis) {
  // Consider RAG context for risk assessment
  if (aiAnalysis.ragContext?.detectedCompanies?.some(c => c.type === 'tech_giant')) {
    return 'Very Low'
  }

  if (aiAnalysis.ragContext?.detectedScams?.some(s => s.scorePenalty <= -30)) {
    return 'Very High'
  }
...
async function extractEnhancedJobDetails(text, aiAnalysis, heuristicAnalysis) {
  // Prefer AI-extracted details, fallback to heuristic extraction
...
function getContextualRecommendations(score, aiAnalysis, heuristicAnalysis) {
  ...
  if (ragContext) {
    if (ragContext.detectedCompanies.length > 0) {
      recommendations.push(`Verified company: ${ragContext.detectedCompanies[0].name} - proceed with confidence`)
    }

    if (ragContext.detectedScams.length > 0) {
      ragContext.detectedScams.forEach(scam => {
        if (scam.scorePenalty <= -30) {
          recommendations.push(`CRITICAL: Known scam pattern detected - ${scam.type}`)
        }
      })
    }
  }
...
function getContextualStrategyTips(score, aiAnalysis, heuristicAnalysis) {
  ...
  } else if (ragContext?.detectedCompanies?.length > 0) {
    tips.push(
...
function calculateEnhancedConfidence(aiAnalysis, heuristicAnalysis) {
  ...
  if (aiAnalysis.ragContext) {
    if (aiAnalysis.ragContext.detectedCompanies.length > 0) confidence += 15
    if (aiAnalysis.ragContext.detectedScams.length > 0) confidence += 10
    if (aiAnalysis.ragContext.professionalSignals.length > 0) confidence += 5
  }
...
function generateEnhancedSummary(score, aiAnalysis, heuristicAnalysis) {
  ...
  if (ragContext?.detectedCompanies?.length > 0) {
    ...
    return `Verified ${company.type} (${company.name}). Professional posting with high legitimacy confidence. ${highRiskCount > 0 ? 'Minor concerns noted but overall appears safe.' : 'No significant risk factors detected.'}`
  }
...
// Keep existing extraction functions (extractJobTitle, extractCompany, extractLocation, extractSalary)
// They remain the same as in your original code but are enhanced with better patterns
...
// Export only async functions for "use server" compliance
...
const ScanService = {
  async getScanJob() {
    return { scanJob, _internal: { buildScanResult, getEnhancedRiskLevel, extractEnhancedJobDetails, calculateEnhancedConfidence } }
  }
}
